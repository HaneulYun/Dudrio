...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case S2C_MOVE: {
		...
		if (move_packet->id < MAX_CLIENTS) {
			...
			if (-1 != my_id) {
				g_clients[my_id].x = move_packet->xPos;
				g_clients[my_id].z = move_packet->zPos;
			}
			if (ci == my_id) {
				if (0 != move_packet->move_time) { // 테스트 클라이언트에서 패킷을 보낸 시간
	...
	break;
	case S2C_CONSTRUCT: break;
	case S2C_DESTRUCT: break;
	case S2C_DESTRUCT_ALL: break;
	case S2C_CHAT: break;
